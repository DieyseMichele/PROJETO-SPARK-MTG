			...
			<table class="w-full whitespace-no-wrap ">
			  <thead>
				<tr>
					<div class="flex justify-center flex-1 lg:mr-32">
					  <div class="relative w-full max-w-xl mr-6 focus-within:text-purple-500">
						...
						<form action="/searchEmprestimo" method="GET">
							@csrf
							<input
							  class="w-full pl-8 pr-2 text-sm text-gray-700 placeholder-gray-600 bg-gray-100 border-0 rounded-md dark:placeholder-gray-500 dark:focus:shadow-outline-gray dark:focus:placeholder-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:placeholder-gray-500 focus:bg-white focus:border-purple-300 focus:outline-none focus:shadow-outline-purple form-input"
							  type="text"
							  placeholder="Digite o nome do usuário, deck ou datas de emprestimo e devolução"
				...
				<tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
				  ...
				  <th class="px-4 py-3">Data de Empréstimo</th>				  
				  <th class="px-4 py-3">Data de Devolução</th>				  
				...
				@foreach ($emprestimos as $emprestimo)
					<tr class="text-gray-700 dark:text-gray-400">
					  <td class="px-4 py-3 text-sm ">
						<a href="" class="font-semibold">{{ $emprestimo->usuario }}</a>
					  </td>
					  <td class="px-4 py-3 text-sm ">
						{{ $emprestimo->deck }}
					  </td>
					  ...
					  <td class="px-4 py-3">
						<div class="flex items-center space-x-4 text-sm">
						  <a data-fancybox data-type="iframe" class=" flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
							aria-label="Edit"  href="/emprestimoAdmin/{{ $emprestimo->id }}/edit" 
						  >
							<svg
							  class="w-5 h-5"
							  aria-hidden="true"
						  ...
						  <form action="/emprestimoAdmin/{{ $emprestimo->id }}" method="POST" onclick="return confirm('Tem certeza que deseja excluir');">
